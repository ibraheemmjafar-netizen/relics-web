...
import { ReadonlyWalletAccount, SUI_CHAINS, SUI_MAINNET_CHAIN, getWallets } from "@mysten/wallet-standard";
...
function getAccountsFromSession(session) {
	...
	return payload.accounts.map((account) => {
		return new ReadonlyWalletAccount({
			...
			chains: SUI_CHAINS,
			features: account.features ? walletAccountFeatures.filter((feature) => account.features?.includes(feature)) : walletAccountFeatures,
	...
	constructor({ name, origin, metadata }) {
		...
		if (typeof window !== "undefined") {
			...
			const onStorage = (event) => {
				if (event.storageArea !== localStorage || event.key !== SLUSH_SESSION_KEY && event.key !== null) return;
...
function registerSlushWallet(name, { origin, metadataApiUrl = METADATA_API_URL } = {}) {
	...
	if (wallets.get().some((wallet) => wallet.id === SUI_WALLET_EXTENSION_ID)) return;
