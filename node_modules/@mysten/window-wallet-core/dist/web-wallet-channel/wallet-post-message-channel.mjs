...
var WalletPostMessageChannel = class WalletPostMessageChannel {
	...
	async verifyJwtSession(secretKey) {
		...
		const addressInSession = session.payload.accounts.find((account) => account.address === requestAddress);
		if (!addressInSession) throw new Error("Requested account not found in session");
		...
		if (addressInSession.features !== void 0 && !requiredFeatures.some((feature) => addressInSession.features.includes(feature))) throw new Error("Requested operation not authorized by session");
		return session;
	}
	sendMessage(payload) {
