...
import { ObjectError, TransactionError } from "../client/errors.mjs";
...
import { fromBase58, fromBase64 } from "@mysten/bcs";
import { chunk } from "@mysten/utils";

//#region src/jsonRpc/core.ts
const MAX_GAS = 5e10;
function mapJsonRpcObjectError(response, requestedObjectId) {
	switch (response.code) {
		...
		case "unknown":
		default: return new ObjectError(response.code, `Unknown error while loading object${requestedObjectId ? ` ${requestedObjectId}` : ""}`, {
			cause: response,
			reason: "unknown",
			objectId: requestedObjectId
		});
...
function isJsonRpcTransactionNotFound(error, digest) {
	if (!(error instanceof JsonRpcError) || error.code !== -32602) return false;
	return error.message === `Invalid Params: Transaction ${digest} not found` || error.message === `Could not find the referenced transaction [TransactionDigest(${digest})].`;
}
...
var JSONRpcCoreClient = class extends CoreClient {
	...
	*/
	async getObjects(options) {
		...
		for (const batch of batches) {
			...
			for (const [idx, object] of objects.entries()) if (object.error) results.push(mapJsonRpcObjectError(object.error, batch[idx]));
	...
	async listOwnedObjects(options) {
		...
		return {
			objects: objects.data.map((result) => {
				if (result.error) throw mapJsonRpcObjectError(result.error);
	...
	*/
	async getTransaction(options) {
		try {
			...
			if (isJsonRpcTransactionNotFound(error, options.digest)) throw new TransactionError("notFound", options.digest, { cause: error });
			throw error;
		}
	}
	/**
	...
	*/
	async simulateTransaction(options) {
		...
		const transactionBytes = data ? data.build({ overrides: { gasData: {
			...
			price: data.gasData.price ?? String(await this.#jsonRpcClient.getReferenceGasPrice({ signal: options.signal })),
	...
	*/
	async listTransactions(options) {
		...
		const page = await this.#jsonRpcClient.queryTransactionBlocks({
			filter: filter && (filter.$kind === "sender" ? { FromAddress: filter.sender } : { MoveFunction: {
		...
		return {
			...
			endCursor: page.data.length ? page.nextCursor ?? page.data[page.data.length - 1].digest : null
		};
	}
	/**
	* @deprecated JSON-RPC APIs are deprecated in the Sui TypeScript SDK. Use `SuiGrpcClient`
	...
	*/
	async listEvents(options) {
		const filter = options.filter ? await resolveEventFilter(this.mvr, options.filter, options.signal) : void 0;
		const { descending, after, before, limit } = resolvePagination(options);
		const cursor = after ?? before;
		const page = await this.#jsonRpcClient.queryEvents({
			query: !filter ? { All: [] } : filter.$kind === "sender" ? { Sender: filter.sender } : filter.$kind === "emitModule" ? { MoveModule: {
				package: filter.package,
				module: filter.module
			} } : filter.$kind === "eventTypeModule" ? { MoveEventModule: {
				package: filter.package,
				module: filter.module
			} } : { MoveEventType: filter.eventType },
			cursor: cursor ? parseEventCursor(cursor) : void 0,
			limit,
			order: descending ? "descending" : "ascending",
			signal: options.signal
		});
		return {
			...
			endCursor: page.data.length && page.nextCursor ? JSON.stringify(page.nextCursor) : page.data.length ? JSON.stringify(page.data[page.data.length - 1].id) : null
		};
	}
	/**
	* @deprecated JSON-RPC APIs are deprecated in the Sui TypeScript SDK. Use `SuiGrpcClient`
	...
	*/
	async resolveNameServiceAddress(options) {
		return { address: await this.#jsonRpcClient.resolveNameServiceAddress(options) };
	}
	/**
	* @deprecated JSON-RPC APIs are deprecated in the Sui TypeScript SDK. Use `SuiGrpcClient`
	* from `@mysten/sui/grpc` or `SuiGraphQLClient` from `@mysten/sui/graphql` instead.
...
function parseEventCursor(cursor) {
	try {
		...
		if (typeof parsed?.txDigest !== "string" || typeof parsed?.eventSeq !== "string") throw new Error("malformed event cursor");
		return parsed;
	} catch {
...
function parseTransactionEffectsJson({ bytes, effects, objectChanges }) {
	...
	const hasGasObject = normalizeSuiAddress(effects.gasObject.reference.objectId) !== normalizeSuiAddress("0x0");
	let lamportVersion = null;
	if (hasGasObject) lamportVersion = effects.gasObject.reference.version;
	else for (const change of changedObjects) if (change.outputVersion && (lamportVersion === null || BigInt(change.outputVersion) > BigInt(lamportVersion))) lamportVersion = change.outputVersion;
	return {
		objectTypes,
		effects: {
			bcs: bytes ?? null,
