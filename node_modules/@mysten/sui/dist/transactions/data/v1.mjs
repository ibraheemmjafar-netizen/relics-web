...
import { JsonU64, ObjectID, TransactionDataSchema, ValidDuringSchema, ValiditySchema, safeEnum } from "./internal.mjs";
...
function expirationToV1(expiration) {
	if (!expiration) return null;
	switch (expiration.$kind) {
		case "Epoch": return { Epoch: Number(expiration.Epoch) };
		case "ValidDuring": return { ValidDuring: expiration.ValidDuring };
		case "Validity": return { Validity: expiration.Validity };
		case "None": return { None: true };
		default: throw new Error(`Unknown transaction expiration: ${JSON.stringify(expiration)}`);
	}
}
function expirationFromV1(expiration) {
	if (!expiration) return null;
	...
	throw new Error(`Unknown transaction expiration in v1 transaction data: ${Object.keys(expiration).join(", ")}`);
}
