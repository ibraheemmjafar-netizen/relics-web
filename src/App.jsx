    ...
    <div className="page">
      ...
      <main>
        <section className="hero" id="top">
          ...
          <div className="hero-art-wrap">
            ...
            <div className="hero-art">
              ...
              <span className="art-corner art-corner-bottom">âˆž</span>
            </div>
            <p className="art-caption"><span /> Live on mainnet Â· art stored on Walrus</p>
          </div>
        </section>

        <section className="ticker-strip" aria-label="Relics facts">
          <div><span className="strip-icon">âœ³</span> THE SEAL IS THE MEME</div>
          <div><span className="strip-icon">âœ³</span> SEND IT TO SOMEONE</div>
          <div><span className="strip-icon">âœ³</span> BREAK IT TOGETHER</div>
          <div><span className="strip-icon">âœ³</span> THE SEAL IS THE MEME</div>
        </section>
        ...
        <section className="story-section section-pad" id="story">
          ...
          <div className="story-cards">
            ...
            <article className="story-card relic-card">
              <div className="card-topline"><span className="mini-icon">â—†</span><span>02 / THE THING</span></div>
              <h3>Relic</h3>
              <p>
        ...
        <section className="loop-section section-pad" id="loop">
          <div className="section-heading-row">
            <div>
              ...
              <h2>Donâ€™t combine your own bags.</h2>
            </div>
            <p className="section-aside">
              Fuse is a request to another wallet. The other person decides
              whether the SUI â€” or the relationship â€” is worth their Relic.
            </p>
          </div>
          <div className="steps">
            <article className="step">
              <span className="step-number">01</span>
              <div className="step-symbol">â—Ž</div>
              <h3>Mint a seal</h3>
        ...
        <section className="manifesto section-pad">
          <div className="manifesto-mark">R<span>Â°</span></div>
          <div>
          ...
          <div className="manifesto-list">
            <div><span>â†’</span><strong>Owned objects</strong><small>Keep it in your wallet</small></div>
            <div><span>â†’</span><strong>Real clay art</strong><small>Stored on Walrus</small></div>
            <div><span>â†’</span><strong>Short history</strong><small>Names and notes persist</small></div>
          </div>
        </section>
        ...
        <section className="coin-section section-pad">
          ...
          <div className="coin-terminal">
            <div className="terminal-header"><span className="terminal-dot" /><span>$RELIC / MAINNET</span><span className="terminal-live">â— LIVE</span></div>
            <div className="terminal-body">
              ...
              <div className="social-links" aria-label="Social links">
                {COIN.telegram ? <a href={COIN.telegram} target="_blank" rel="noreferrer">Telegram â†—</a> : null}
                {COIN.twitter ? <a href={COIN.twitter} target="_blank" rel="noreferrer">X / @SuiRelics â†—</a> : null}
                {COIN.buyUrl ? <a href={COIN.buyUrl} target="_blank" rel="noreferrer">Buy $RELIC â†—</a> : null}
                {COIN.chartUrl ? <a href={COIN.chartUrl} target="_blank" rel="noreferrer">Chart â†—</a> : null}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="app-section section-pad" id="app">
          <div className="app-heading">
            ...
            <div className="account-state">
              ...
              {account ? `${account.address.slice(0, 6)}â€¦${account.address.slice(-4)}` : "Connect a wallet to begin"}
            </div>
          </div>
          ...
          <div className="app-grid">
            <section className="app-panel mint-panel">
              ...
              <button className="button button-primary wide-button" type="button" disabled={!account} onClick={() => mint().catch((e) => setStatus(e.message))}>
                {account ? "Mint a Relic" : "Connect wallet to mint"} <span>â†—</span>
              </button>
            </section>
            ...
            <section className="app-panel offer-panel">
              ...
              <select value={offerId} onChange={(e) => setOfferId(e.target.value)}>
                {relics.map((r) => <option key={r.id} value={r.id}>#{r.serial} {r.name} Â· gen {r.gen}</option>)}
              </select>
              <label>Their wallet</label>
              <input placeholder="0xâ€¦" value={toAddr} onChange={(e) => setToAddr(e.target.value)} />
          ...
          <section className="wallet-section">
            ...
            {relics.length === 0 ? (
              <div className="empty-state"><span>â—‡</span><p>{account ? "No Relics in this wallet yet." : "Connect a wallet to see your Relics."}</p></div>
            ) : (
              <div className="relic-grid">
                {relics.map((r) => (
                  <article key={r.id} className="owned-card">
                    ...
                    <button className="small-button" type="button" onClick={() => stamp(r).catch((e) => setStatus(e.message))}>Stamp gen art â†—</button>
                  </article>
                ))}
              </div>
            )}
          ...
          <section className="wallet-section incoming-section">
            ...
            {deals.length === 0 ? (
              <div className="empty-state compact"><span>â†—</span><p>Nobody has asked you to fuse yet.</p></div>
            ) : (
              <div className="deal-controls">
                ...
                <label>Request<select value={dealId} onChange={(e) => setDealId(e.target.value)}>{deals.map((d) => <option key={d.id} value={d.id}>From {String(d.from).slice(0, 10)}â€¦ Â· {(Number(d.pay) / 1e9).toFixed(3)} SUI</option>)}</select></label>
                <label>Your Relic to burn<select value={mineId} onChange={(e) => setMineId(e.target.value)}>{relics.map((r) => <option key={r.id} value={r.id}>#{r.serial} {r.name} Â· gen {r.gen}</option>)}</select></label>
                <div className="deal-actions">
                  <button className="button button-primary" type="button" onClick={() => acceptDeal().catch((e) => setStatus(e.message))}>Accept Â· take SUI</button>
                  <button className="button button-outline" type="button" onClick={() => rejectDeal().catch((e) => setStatus(e.message))}>Reject Â· return it</button>
                </div>
              </div>
            )}
          </section>
